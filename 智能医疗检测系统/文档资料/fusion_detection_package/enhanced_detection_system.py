# -*- coding: utf-8 -*-
"""
Enhanced Nodule Detection System
增强版结节检测系统 - 提供高级的结节检测算法和可视化功能
"""
...
class EnhancedNoduleDetectionSystem:
    ...
    def advanced_nodule_detection(self, stress_grid, timestamp):
        ...
        for prop in props:
            if prop.area >= self.detection_params['min_nodule_area']:
                ...
                # 计算风险评分
                individual_mask = (labeled_mask == prop.label).astype(int)
                risk_score = self.calculate_risk_score(area, circularity, mean_intensity, 
                                                     stress_grid, individual_mask)
    ...
    def calculate_risk_score(self, area, circularity, intensity, stress_grid=None, nodule_mask=None):
        """计算结节风险评分"""
        try:
            # 尝试使用高级概率预测系统
            if (hasattr(self, 'advanced_probability_system') and 
                stress_grid is not None and nodule_mask is not None):
                
                # 使用高级系统预测概率
                probability = self.advanced_probability_system.predict_probability(
                    stress_grid=stress_grid,
                    nodule_mask=nodule_mask,
                    timestamp=len(self.nodule_history['timestamps'])
                )
                return probability
            else:
                # 回退到传统方法
                return self._traditional_risk_score(area, circularity, intensity)
                
        except Exception as e:
            print(f"高级概率预测失败，使用传统方法: {e}")
            return self._traditional_risk_score(area, circularity, intensity)
    
    def _traditional_risk_score(self, area, circularity, intensity):
        """传统风险评分方法（备用）"""
        # 基于面积、圆形度和强度的综合评分
    ...
    def create_enhanced_visualization(self, df, output_path='enhanced_nodule_detection.gif', max_frames=50):
        ...
        for idx in tqdm(range(total_frames), desc="生成动画帧"):
            ...
            if len(self.nodule_history['areas']) > 0:
                ...
                max_risk = max(self.nodule_history['risk_scores'])
                detection_rate = sum(1 for c in self.nodule_history['count'] if c > 0) / len(self.nodule_history['count'])
