...
class EnhancedNoduleDetectionSystem:
    ...
    def advanced_nodule_detection(self, stress_grid, timestamp):
        ...
        for prop in props:
            if prop.area >= self.detection_params['min_nodule_area']:
                ...
                # 计算风险评分 - 为每个结节创建单独的掩码
                individual_mask = np.zeros_like(normalized, dtype=np.uint8)
                individual_mask[labeled_mask == prop.label] = 1
                
                risk_score = self.calculate_risk_score(
                    area, circularity, mean_intensity, 
                    stress_grid=normalized, 
                    nodule_mask=individual_mask
                )
    ...
    def calculate_risk_score(self, area, circularity, intensity, stress_grid=None, nodule_mask=None):
        """使用高级概率预测系统计算结节风险评分"""
        try:
            if stress_grid is not None and nodule_mask is not None:
                # 使用高级概率预测系统
                prediction_result = self.advanced_probability_system.predict_probability(
                    matrix=stress_grid,
                    nodule_mask=nodule_mask,
                    elastography_data=None  # 传统检测系统没有弹性成像数据
                )
